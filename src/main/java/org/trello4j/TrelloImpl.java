...
public class TrelloImpl implements Trello {
    ...
    public Board createBoard(String name) {
        final String url = TrelloURL
                .create(apiKey, TrelloURL.BOARD_CREATE_URL)
                .token(token)
                .build();
    ...
    @Override
    public void addMemberToBoard(String boardId, String memberId) {
        boolean addedMember = false;
        List<Member> membersByBoard = getMembersByBoard(boardId);
        for (Member member : membersByBoard) {
            if (member.getUsername().equals(memberId) || member.getId().equals(memberId)) {
                addedMember = true;
                break;
            }
        }
        ...
        if (!addedMember) {
            final String url = TrelloURL
                    .create(apiKey, TrelloURL.BOARD_ADD_MEMBER_URL, boardId, memberId)
                    .token(token)
                    .build();
            Map<String, String> keyValueMap = new HashMap<String, String>();
//        keyValueMap.put("idMember", memberId);
            keyValueMap.put("type", "normal");
    ...
    public void deleteCard(String cardId) {
        final String url = TrelloURL
                .create(apiKey, TrelloURL.CARD_DELETE_URL, cardId)
                .token(token)
                .build();
        doDelete(url);
    }
    ...
    public void addMemberToCard(String cardId, String memberId) {
        final String url = TrelloURL
                .create(apiKey, TrelloURL.CARD_MEMBER_URL, cardId)
                .token(token)
                .build();
        Map<String, String> keyValueMap = new HashMap<String, String>();
    ...
    public void archiveCard(String cardId) {
        final String url = TrelloURL
                .create(apiKey, TrelloURL.CARD_ARCHIVE_URL, cardId)
                .token(token)
                .build();
        Map<String, String> keyValueMap = new HashMap<String, String>();
        keyValueMap.put("closed", "true");
    ...
    @Override
    public Checklist createChecklist(String idCard, String name, Map<String, String> keyValueMap) {
        validateObjectId(idCard);

        final String url = TrelloURL
                .create(apiKey, TrelloURL.CHECKLIST_POST_URL, idCard)
                .token(token)
                .build();
        if (keyValueMap == null) keyValueMap = new HashMap<String, String>();
        //if (keyValueMap.containsKey("name")) keyValueMap.remove("name");
        keyValueMap.put("name", name);
        keyValueMap.put("idList", idCard);
        keyValueMap.put("idList", idCard);
    ...
    @Override
    public org.trello4j.model.List createList(String listName, String boardId) {
        validateObjectId(boardId);

        final String url = TrelloURL
                .create(apiKey, TrelloURL.LIST_CREATE_URL)
                .token(token)
                .build();
        Map<String, String> keyValueMap = new HashMap<String, String>();
        keyValueMap.put("name", listName);
    ...
    @Override
    public org.trello4j.model.List createList(String listName, String boardId, int order) {
        validateObjectId(boardId);
        ...
        keyValueMap.put("name", listName);
        keyValueMap.put("idBoard", boardId);
        int position = order * 32;
        keyValueMap.put("pos", String.valueOf(position));
    ...
    @Override
    public void restoreList(String listId) {
        validateObjectId(listId);
    ...
    @Override
    public void setListOrder(String listId, int order) {
        validateObjectId(listId);
    ...
    @Override
    public CheckItem createCheckItem(String checkListId, String itemName, boolean isSelected) {
        validateObjectId(checkListId);
    ...
    private InputStream doRequest(String url, String requestMethod, Map<String, String> map) {
        ...
        while (true) {
            ...
            if (tryCount <= 5) {
                ...
                throw new TrelloException("Failed to get response after " + tryCount + " tries.");
            }
        }
    }
